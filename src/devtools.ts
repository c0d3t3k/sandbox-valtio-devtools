...
export const useProxyDevtools = <T>(proxy: T, name: string) => {
  const options = {
    name
  };
  ...
  useEffect(() => subscribe(proxy, () => {}), []);

  // Use hook to pipe value of atom to observable.
  const atom$ = useObservable((input$) => input$.pipe(map(([x]) => x)), [
    atomCurrentValue
  ]);

  // Flag determines whether the last state update was a 'Time Travel' event
  const [wasTriggeredByDevtools, setWasTriggeredByDevtools] = useState(
    () => false
  );
  ...
  const initDevtools = () => {
    ...
    if (!devtools) {
      console.error(
        "Jotai Devtools plugin: Cannot find Redux Devtools browser extension. Is it installed?"
      );
      return atom;
    }
